...
void ETH_SendResponseFrame(ETH_TxDescriptor *DMATxDesc, uint16_t Framelength);
...
int main(void)
{
	...
	while(1)
	{
		...
		if((ETH->DMASR & RECEIVE_FINISHED) == RECEIVE_FINISHED)
		{
			...
			Framelength = ((DMARxDesc->Status & RX_FRAMELENGTH_MAX) >> RX_FRAMELENGTH_OFFSET) - 4U; // We substract 4 because of CRC
			...
			if (Framelength < 83)
			{
				...
				/* Send response */
				ETH_SendResponseFrame(DMATxDesc, sizeof(struct ETH_Header) + swap_uint16(iphdr.TotalLength));
			}

			else
			{
				/* Send error frame */
				ETH_SendErrorFrame(DMATxDesc, SIZE_OF_HEADERS + 2U);
			}
...
void ETH_SendErrorFrame(ETH_TxDescriptor *DMATxDesc, uint16_t Framelength)
{
	...
	DMATxDesc->BufSize = ((Framelength << 16) & TX_DESC_BUF2SIZE_MAX);
...
void ETH_SendResponseFrame(ETH_TxDescriptor *DMATxDesc, uint16_t Framelength)
{
	...
	/* Copy IP header as well as data from Rx buffer to Tx buffer */
	memcpy((uint8_t *)(DMATxDesc->Buf1Addr + sizeof(struct ETH_Header)), (uint8_t *)&iphdr, sizeof(struct IP_Header));
	memcpy((uint8_t *)DMATxDesc->Buf1Addr + SIZE_OF_HEADERS + 2U,
